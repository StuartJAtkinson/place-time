...
export interface SearchSource {
  ...
  endpoint?: string;
  description: string;
  timeRange: { minYear: number; maxYear: number };
...
export async function embedText(
  ...
  const data: OllamaEmbedResponse = await response.json() as OllamaEmbedResponse;
...
export class EmbeddingSearchPipeline {
  ...
  private index: Map<string, EmbeddingEntry[]> = new Map(); // layerId → entries
  ...
  async searchCell(
    ...
    const {
      entityTypes = ['boundary', 'administrative', 'geological'],
      minConfidence = 0.6,
      limit = 10
    } = options;
    ...
    const queries = entityTypes.map(type =>
      this.buildQuery(coord, { entityType: type })
    );

    // Embed all queries
    const queryEmbeddings = await embedBatch(queries, this.embeddingModel);

    const results: BoundarySearchResult[] = [];
   ...
   */
  async indexEntity(entry: Omit<EmbeddingEntry, 'embedding'>): Promise<void> {
    const ollamaReady = await this.checkOllama();
    ...
    const fullEntry: EmbeddingEntry = { ...entry, embedding };
 ...
 */
export function makeCliopatriaSource(baseUrl = 'https://cliopatria.snalS.org'): SearchSource {
  return {
    ...
    search: async (query: string, year?: number) => {
 ...
 * Free, no API key required.
 */
export function makeOpenDomesdaySource(): SearchSource {
  return {
 ...
 * Free, no API key required.
 */
export function makeGeofabrikSource(): SearchSource {
  return {
    id: 'admin:geofabrik',
    name: 'Geofabrik OSM',
 ...
 * Free, no API key required.
 */
export function makeZenodoGeologySource(): SearchSource {
  return {
    id: 'geology:zenodo',
    ...
    search: async (query: string, _year?: number) => {
 ...
 */
export function createSearchPipeline(): EmbeddingSearchPipeline {
