 ...
 */
export function pointToH3(lat, lng, resolution = DEFAULT_RESOLUTION) {
    return latLngToCell(lat, lng, resolution);
}
/**
 * Convert H3 cell ID to [lat, lng] centroid.
 */
export function h3ToCentroid(cellId) {
 ...
 */
export function polygonToH3Cells(geometry, resolution = DEFAULT_RESOLUTION) {
    if (geometry.type === 'MultiPolygon') {
        const cells = [];
        for (const poly of geometry.coordinates) {
            const ring = poly[0];
            const result = polygonToCells(ring, resolution);
 ...
 */
export function indexLayerByH3(layer, resolution = DEFAULT_RESOLUTION) {
    const index = new Map();
 ...
 */
export function queryCellAtYear(cellId, year, boundaries, options) {
    return boundaries.filter(b => {
        ...
        if (options?.minPriority && b.priority < options.minPriority)
            return false;
 ...
 */
export function executeTimeQuery(query, layers, boundaries) {
 ...
 */
function findPlaceNameAtPoint(point, _layers) {
 ...
 */
function findGeologicalAtPoint(point, layers) {
    const [lng, lat] = point;
 ...
 */
export function buildHexGrid(minLon, minLat, maxLon, maxLat, resolution = DEFAULT_RESOLUTION) {
    // Get center cell
