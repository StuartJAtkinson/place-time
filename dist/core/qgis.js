 ...
 */
export function generateQlrLayer(opts) {
    const { layer, dataPath, crs = 'EPSG:4326' } = opts;
    const style = (layer.style ?? defaultStyleForLayer(layer.id));
    if (!layer.features[0]) {
...
function defaultStyleForLayer(layerId) {
 ...
 */
export function writeQlrFile(opts, outputPath) {
 ...
 */
export function generateQlrProject(layers, dataDir, outputPath) {
